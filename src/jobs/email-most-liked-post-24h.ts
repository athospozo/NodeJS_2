...
export const enviarEmailsDestaqueJob = cron.schedule(
  env.CRON_SCHEDULE,
  async () => {
        ...
        try {
          ...
          if (resultado != null && resultado.post != null) {
            ...
            await sendEmailUseCase.execute({
              ...
              message: mostLikedPostTextTemplate(
                usuario.name,
                resultado.post._count.likes,
                resultado.post.titulo,
              ),
              html: mostLikedPostHtmlTemplate(
                usuario.name,
                resultado.post._count.likes,
                resultado.post.titulo,
              ),
            ...
            logger.info(
              { targetId: usuario.publicId },
              'Most liked post email sent',
            )
          }
        } catch (error) {
          console.error(
            `Erro ao processar usuário ${usuario.name}: ${usuario.publicId}:`,
            error,
          )
        }
      }
